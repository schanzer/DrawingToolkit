...
i1.loadFile("test_files/icon.gif", "gif",black, true);
BWmask.loadFile("test_files/BWMask.gif", "gif",black, true);
ColorMask.loadFile("test_files/ColorMask.png", "png",red, true);
AlphaMask.loadFile("test_files/AlphaMask.png", "png",green, true);
...
var drawing  =  new Array( { api: "erase"
                          ...
                          for(var i=1; i<points.length; i++) path.lineTo(points[i].x,points[i].y);
                          dc.drawPath(path, 320, 130);} }
                          ,{ api: "draw-point"
